...
FLEXT API - High-Performance REST API with FastAPI
...
2. Confirm parent MRO chain via `pyproject.toml` `dependencies` filtered by `flext-*` (excluding `flext-api` self).
...
- Public extensions exposed by this project: `FlextApiErrors`, `FlextApiStorage`, `FlextApiClient`, `FlextApiConstants`, `FlextWebClientImplementation`, `FlextApiUtilitiesSerializers` (+24 more).
