...
class FlextApiUtilitiesSettingsManager:
    ...
    def _normalize_value(
        ...
        *,
        value: t.Scalar,
    ) -> p.Result[t.JsonValue | t.StrMapping]:
        ...
        if key == "headers" and isinstance(value, Mapping):
            ...
            if validated_result.failure:
                return r[t.JsonValue | t.StrMapping].fail(validated_result.error)
            return r[t.JsonValue | t.StrMapping].ok(validated_result.value)
        if key == "headers" and isinstance(value, str):
            ...
            if parsed_result.failure:
                return r[t.JsonValue | t.StrMapping].fail(parsed_result.error)
            return r[t.JsonValue | t.StrMapping].ok(parsed_result.value)
        if key in {"log_requests", "log_responses", "verify_ssl"}:
            ...
            if bool_result.failure:
                return r[t.JsonValue | t.StrMapping].fail(bool_result.error)
            return r[t.JsonValue | t.StrMapping].ok(bool_result.value)
        return r[t.JsonValue | t.StrMapping].ok(u.normalize_to_container(value))
    ...
    def _build_client_config(
        ...
        processed: MutableMapping[str, t.JsonValue | t.StrMapping] = {}
