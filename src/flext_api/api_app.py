...
class FlextApiAppConfig:
    ...
    def __init__(self, settings: FlextApiSettings | None = None):
        """Initialize application configuration."""
        if settings is None:
            settings_result = create_api_settings()
            if not settings_result.success:
                raise RuntimeError(f"Failed to create settings: {settings_result.error}")
        ...
        try:
            ...
            if hasattr(app.state, "storage") and app.state.storage:
                try:
                    ...
                    await app.state.storage.set(test_key, {"timestamp": health_data["timestamp"]})
                    await app.state.storage.delete(test_key)
...
def setup_api_endpoints(app: FastAPI, config: FlextApiAppConfig) -> None:
    ...
    async def api_info() -> dict[str, object]:
        ...
        return {
            ...
            "environment": {
                "name": config.settings.environment,
                "debug": config.settings.debug,
                "host": config.settings.api_host,
                "port": config.settings.api_port,
            },
            "features": {
                "caching": config.settings.enable_caching,
...
def create_flext_api_app(config: FlextApiAppConfig | None = None) -> FastAPI:
    ...
    app = FastAPI(
        ...
        openapi_url="/openapi.json" if config.settings.debug else None,
    )
    ...
    logger.info("FLEXT API application created successfully",
               title=config.get_title(),
               version=config.get_version(),
               environment=config.settings.environment)
...
def create_flext_api_app_with_settings(**settings_overrides) -> FastAPI:
...
def run_development_server(
    ...
    if port is None:
        port = app.state.config.settings.api_port
...
try:
    ...
    __all__ = ["app", "create_flext_api_app", "run_development_server", "run_production_server", "storage"]

except Exception as e:
    logger.error("Failed to create default app instance", error=str(e))
    # Create a minimal app for error cases
    app = FastAPI(title="FLEXT API - Error", description="Failed to initialize properly")
    storage = None
    error_message = str(e)
