...
FLEXT API development follows the architectural patterns established in **[flext-core](../flext-core/README.md)**, emphasizing **HTTP-specific** implementations of FLEXT patterns and **quality standards**. All development must align with the broader FLEXT ecosystem patterns.
...
5. **Ecosystem Integration** - Seamless HTTP integration with all 33 FLEXT projects
...
```bash
ruff check flext-api
PYTHONPATH=flext-api/src mypy -p flext_api
pyright flext-api/src/flext_api
pytest -q flext-api/tests/unit -k "client or config or utilities"
```
