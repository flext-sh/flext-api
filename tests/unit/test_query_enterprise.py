...
class TestFlextApiQueryBuilderRealWorldScenarios:
    ...
    def test_query_builder_reuse(self):
        ...
        # Admin users query
        REDACTED_LDAP_BIND_PASSWORD_result = (base_query
        ...
        REDACTED_LDAP_BIND_PASSWORD_role_filter = next(f for f in REDACTED_LDAP_BIND_PASSWORD_result["filters"] if f["field"] == "role")
        assert REDACTED_LDAP_BIND_PASSWORD_role_filter["value"] == "REDACTED_LDAP_BIND_PASSWORD"
        ...
        assert len(regular_result["filters"]) > len(REDACTED_LDAP_BIND_PASSWORD_result["filters"])