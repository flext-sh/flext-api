...
class TestFlextApiStorageFocused:
    ...
    def test_storage_make_key_method(self) -> None:
        ...
        # Test default namespace (since config doesn't actually change namespace)
    ...
    def test_storage_namespace_isolation(self) -> None:
        """Test that storage instances are isolated from each other."""
        ...
        # Each storage instance maintains its own data
        result1 = storage1.get("shared_key")
        result2 = storage2.get("shared_key")

        # Each storage keeps its own values (instances are isolated)
    ...
    def test_storage_get_with_structured_data(self) -> None:
        ...
        # Simulate data with timestamp structure
        storage_key = storage._make_key("structured_key")
        storage._storage[storage_key] = {
            "value": "structured_value",
            "timestamp": "2025-01-01T00:00:00Z",
        }
        storage._data["structured_key"] = "structured_value"

        # Get should extract the value from structured data
        result = storage.get("structured_key")
        assert result.is_success
        assert result.unwrap() == "structured_value"

    def test_storage_get_with_direct_data(self) -> None:
        """Test storage get operation with direct data (not structured)."""
        storage = FlextApiStorage()

        # Simulate direct data storage
        storage_key = storage._make_key("direct_key")
        storage._storage[storage_key] = "direct_value"

        result = storage.get("direct_key")
        assert result.is_success
