...
from flext_api import FlextApiServicesTransports, m, p, t
...
class TestsFlextApiTransportsCharacterization:
    ...
    @pytest.fixture
    def transport(self) -> FlextApiServicesTransports.FlextWebTransport:
        """Return a fresh, disconnected HTTP transport."""
        return FlextApiServicesTransports.FlextWebTransport()

    def test_connect_rejects_empty_url(
        self, transport: FlextApiServicesTransports.FlextWebTransport
    ) -> None:
    ...
    def test_connect_accepts_url_and_echoes_it(
        self, transport: FlextApiServicesTransports.FlextWebTransport, url: str
    ) -> None:
    ...
    def test_connect_accepts_documented_client_options(
        self,
        transport: FlextApiServicesTransports.FlextWebTransport,
        options: t.Api.RequestKwargs,
    ) -> None:
    ...
    def test_concrete_transport_is_not_a_protocol_member(self) -> None:
        """The concrete httpx transport no longer lives in the protocols facade."""
        tm.that(hasattr(p.Api, "FlextWebTransport"), eq=False)
        tm.that(hasattr(FlextApiServicesTransports, "FlextWebTransport"), eq=True)
