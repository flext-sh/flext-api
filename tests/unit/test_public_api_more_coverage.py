...
async def test_storage_set_get_via_factory() -> None:
    ...
    assert got.success and got.data == "v"
...
def test_deprecated_create_api_service_and_client_paths() -> None:
    ...
    assert ok.success and ok.data is not None
...
def test_app_health_storage_error_and_nonawaitable_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    # Normal app
    ...
    class BadStorage:
        def set(self, *_args, **_kwargs):  # no awaitable
    ...
    body2 = r2.json()
    assert body2.get("services", {}).get("storage", {}).get("last_check") == "successful"
