...
class TestFlextApiExceptionsComprehensive:
    ...
    def test_http_timeout_error(self) -> None:
    ...
    def test_client_error_invalid_status_code(self) -> None:
        """Test ClientError with invalid status code (not 4xx)."""
        with pytest.raises(ValueError, match="Client error status code must be 4xx"):
            FlextApiExceptions.ClientError("Test error", status_code=500)

    def test_server_error_invalid_status_code(self) -> None:
        """Test ServerError with invalid status code (not 5xx)."""
        with pytest.raises(ValueError, match="Server error status code must be 5xx"):
            FlextApiExceptions.ServerError("Test error", status_code=400)
