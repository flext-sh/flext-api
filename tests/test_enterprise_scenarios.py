        ...
        async def validate_complex(request: Request):  # noqa: ANN202
            ...
            validator = (FlextApiValidator()
                        ...
                        .validate_choices("role", user_data.role,
                                        ["REDACTED_LDAP_BIND_PASSWORD", "manager", "employee"])
...
class TestEnterpriseAuthentication:
    ...
    def test_authentication_flow(self) -> None:
        ...
        @app.post("/REDACTED_LDAP_BIND_PASSWORD-only")
        @flext_api_handle_errors()
        @flext_api_authorize_roles("REDACTED_LDAP_BIND_PASSWORD", "manager")
        async def REDACTED_LDAP_BIND_PASSWORD_endpoint(request: Request):  # noqa: ANN202
            user = request.user
            return flext_api_success_response(
                data={"REDACTED_LDAP_BIND_PASSWORD_action": "completed", "user": user.get("user_id")}
            )
