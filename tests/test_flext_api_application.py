...
class TestApplicationDecorators:
    ...
    async def test_require_auth_decorator_success(self) -> None:
        ...
        @flext_api_require_auth(roles=["admin", "user"])
    ...
    async def test_require_auth_decorator_insufficient_permissions(self) -> None:
        ...
        @flext_api_require_auth(roles=["admin"])
        async def admin_function(context: dict[str, Any] | None = None) -> str:
            return "Admin only"

        context = {
            "user_id": "user-123",
            "roles": ["user"],  # Doesn't have admin role
        }

        result = await admin_function(context=context)
